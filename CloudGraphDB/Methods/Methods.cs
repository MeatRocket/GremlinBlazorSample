...
namespace AngryMonkey.Cloud.GraphDB
{
    public class CloudGraphDbClient
    {
        ...
        public async Task<List<VertexRecord>> GetOutVertecies(string partitionKey, Guid id)
        {
            ...
            StringBuilder builder = new($"g.V(\"{id}\").has(\"PartitionKey\",\"{partitionKey}\").inE().outV()");
        ...
        {
            List<VertexRecord> VRList = new();
            StringBuilder builder = new($"g.V(\"{id}\").has(\"PartitionKey\",\"{partitionKey}\").outE().inV()");
        ...
        public async Task<List<EdgeRecord>> GetInEdges(Guid id)
        {
            List<EdgeRecord> EGList = new();
            StringBuilder builder = new($"g.V(\"{id}\").inE()");

            ResultSet<dynamic> resultSet = await Client.SubmitAsync<dynamic>(builder.ToString());
            foreach (dynamic edge in resultSet)
            {
                EdgeRecord v = EdgeRecord.Parse(edge);
                EGList.Add(v);
            }
            return EGList;
        }

        public async Task<List<EdgeRecord>> GetOutEdges(Guid id)
        {
            List<EdgeRecord> EGList = new();
            StringBuilder builder = new($"g.V(\"{id}\").outE()");
